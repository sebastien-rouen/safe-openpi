...
- **Bandeau support (rotation) dans Scrum et Kanban** : affiche la semaine courante de support (avatars + prenoms) et la passation (semaine suivante, discret) via `_renderScrumSupportBanner()` et `_renderKanbanSupportBanner()`. Utilise la nouvelle fonction utilitaire `_getSupportRosterData(teams)` partagee depuis `support.js` (`support.js`, `scrum.js`, `kanban.js`, `index.html`, `views.css`)
